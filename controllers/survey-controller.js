...
const createSurvey = async (req, res) => {
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      if (data.filter.location && data.filter.location.length > 0) {
        for (let j = 0; j < data.filter.location.length; j++) {

          const locationx = data.filter.location[j];
          console.log(locationx)
          const { latitude, longitude, radius } = locationx;

          // Check if user'xs location exists
          if (users[i].address_lat && users[i].address_lng) {
            const userLat = users[i].address_lat;
            const userLng = users[i].address_lng;

            // Check if user's location lies within the specified radius
            console.log("userLat, userLng, latitude, longitude, radius")
            console.log(userLat + userLng + latitude + longitude + radius)
            if (liesWithinTheRadius(userLat, userLng, latitude, longitude, radius)) {
              console.log("HERE")
              spn(data['projectName'], 'Please Fill this New Survey to Get More Reward Points', users[i]._id,)
  ...
  try {
    ...
    for (let i = 0; i < finalArray.length; i++) {
      ...
      if (element.filter.location && element.filter.location.length > 0) {
        for (let j = 0; j < element.filter.location.length; j++) {
    ...
    for (let i = 0; i < trulyFinalArray.length; i++) {
      ...
      if (!userResponse) {
        ...
        if (element.expirationDate !== null && new Date(element.expirationDate) < new Date()) {
          pendingSurveys.push(element)
          // expirationDate is not null and is less than the current date
        }
